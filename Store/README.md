...
Store/
  ...
  metadata/
    en-US/                      one file per App Store Connect field
      name.txt                  30 char limit   — 12 used
      subtitle.txt              30              — 25
...
The repo keeps its `WeatherStar` internals — project, package and type names — on
purpose. Renaming an app is about what the store and the user see; churning every
symbol would be a large diff for no benefit. A public, MIT, disclaimed fan port is the
same posture `ws4kp` has held for years.
...
**2. Location permission.** The app used to ask for a location fix on launch whenever
`locationMode` was `.device`, including on a first run with nothing saved — which is a
permission prompt the user never asked for. It now only resolves a device fix off the
back of an explicit choice, and skipping location during onboarding sets manual mode
rather than leaving the app pointed at a device fix it will never request. Worth knowing
if a reviewer asks why location is optional.

## What still needs you

The listing itself is done — everything below was pushed with `Tools/asc.py` and read
back out of the API to confirm it landed.

| Thing | Why it is not done |
|---|---|
| A build on each version | Nothing is attached yet. Build 10 went to TestFlight for iOS; tvOS and macOS were archived but may not have been uploaded. `Tools/archive.sh` builds all three. |
| Price set to Free | The description says free, no in-app purchases and no subscription. The pricing page still has to agree. |
| The `VISION_OS` platform | There is a 1.0 version on the record and it now has the same metadata as the rest, but there is no visionOS target in the project, so no build will ever arrive and it has no screenshots. Either drop the platform or build for it. |

Settled, and verified against the API rather than assumed:

- **Name and subtitle** — "weatherfeels" and "A 90's Retro forecast app", both set in the
  browser. `Tools/asc.py text` overwrites from `Store/metadata/en-US/*.txt`, so those
  files are kept in sync deliberately; change them there or the next run reverts you.
- **All four platforms already existed** on the record — IOS, TV_OS, MAC_OS, VISION_OS.
  An earlier note here claimed tvOS and macOS had to be added by hand. They did not.
- **Screenshots** — 23 uploaded, and both guessed display types were right:
  `APP_IPHONE_67` takes the 6.9" iPhone, `APP_IPAD_PRO_3GEN_129` the 13" iPad.
- **Age rating** — 23 answers, all "none"/false, pushed through the API. Another earlier
  note said this could not be scripted. It can.
- **Review contact** — Kevin Holland, 844-247-4274, hollandkevin@icloud.com, no demo
  account, plus the reviewer notes from `metadata/review-notes.md`.
- **Free, no in-app purchases, no subscription.** Which matters for more than the pricing
  page — see the name section above.
- **Support and privacy URLs** — `docs/support.md` and `docs/privacy.md` in this repo.
  That is why the repo is public: Apple checks both load for an anonymous visitor.
- **`whatsNew` is not set** and cannot be, on a version that has never shipped. It starts
  applying with the first update.

## Pushing it

```sh
export ASC_KEY_ID=LFCSNDNP56           # the one that works; RJ5D99L9C9 returns 401
export ASC_ISSUER_ID=<uuid>            # not stored in this repo

Tools/asc.py status                    # read-only. run this first, and again after.
...
Tools/asc.py review                    # review contact and notes
